...
## Brand Systems (LOCKED — not exploration candidates)

> A brand system is **brand canon**, not a taste option. When the user names a brand registered in this section, **skip the 3-preview exploration entirely and build directly in that system.** The whole point of a brand system is that it is not re-decided per deck. Offering alternatives here is a defect, not a service.
> Everything else in the deck still applies: one idea per slide, the anti-slop rules, the fixed 16:9 stage.

### 13. Your brand system (template — register your own)

This section ships empty on purpose: your brand belongs to you. To lock a brand in, copy the template below into this file, fill it with YOUR real values (or ask the AI to extract them from your site/brand book), and from then on say the brand's name to skip style exploration.

```markdown
### NN. [Brand name] ([owner / when it applies])

**Vibe:** [3-6 words — e.g. "Dark, editorial, high-contrast, warm neon".]

**Typography:**
- Display: `[font]` ([weight]) — [rules, e.g. "always uppercase"]
- Body: `[font]` ([weights])
- `[the <link> tag or self-hosted note]`
...
:root {
    --bg: #......;
    --ink: #......;
    --accent: #......;   /* the ONE accent — keywords, numbers, CTAs */
    /* ...the full token set, with comments on what each is for */
}
```

**Signature Elements:**
- [3-5 rules that make the brand recognizable: the emphasis pattern,
  the kicker style, the one thing that must never be recolored,
  what is forbidden (e.g. "no second accent — it breaks the system")]
```

Rules that make a brand system work: **one accent doing all the emphasis** · a fixed emphasis pattern (e.g. one accented `<span>` per title, never the whole title) · named forbidden moves · and no per-deck re-decision.
...
| *(your brand)* | *(your display font)* | *(your body font)* | *(your source)* |
