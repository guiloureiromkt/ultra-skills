# reference-boards — design the page as IMAGES before writing code (Fase 2 do ultra-site)
...
`scripts/gen_images.py` (modelo forte: `nano` = google/nano-banana-pro,
resolution 2K). 6 sections = 6 boards. Never one tall full-page image (detail
gets mushy and per-section composition variety dies). Aspect: 16:9, landscape.

Submit all boards in ONE batch (uma chamada do script com todas as entradas no
spec JSON) into the project's `refs/` directory (working artifact — NOT
`assets/`, que é só o que o site referencia).

## The combinatorial pick (commit BEFORE prompting)

To avoid the AI defaults, pick ONE option per category, write the pick into
`design-brief.md`, and hold it across ALL boards. Do not mash categories;
...
## Palette bans (hard, also enforced by the mechanical gate)
