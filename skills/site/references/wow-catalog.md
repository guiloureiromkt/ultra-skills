# wow-catalog — Tier-1 experience techniques (escolhe no brief · constrói no motion pass)
...
first idea that comes to mind. Every technique here leans on generated media
(Replicate): the wow is bespoke media responding to input, not a CSS trick.
...
- **B2 — Grade-shift interaction pair.** Two renders of the SAME composition
  ...
  effect.
- **B3 — 3D subject scene.** Approved hero image → modelo image-to-3D no
  Replicate → GLB → three.js scene with scroll-driven camera orbit + cursor
  tilt. Spectacle tier.
...
- Input → response latency feels physical: scrub via ScrollTrigger progress
  (scrub: 0.5-1), cursor com spring/lerp dentro de `requestAnimationFrame`
  (vanilla; em React, `useMotionValue` — nunca `useState` por frame). Never
