# asset-system — the generated visual system (Fase 3 do ultra-site · motor: Replicate)

The whole point of this skill: sites built here look expensive because **as
much of the visual layer as possible is bespoke-generated** — not stock, not
icon fonts, not CSS approximations. One brief → one coherent generated visual
system. Treat this phase as designing a brand asset kit, not "getting a hero
image".

Ferramentas (Replicate · pagamento por uso · avisar custo estimado antes de
lotes grandes ou vídeo):

- **Imagem:** `scripts/gen_images.py` — `nano` (google/nano-banana-pro ·
  default · direção de arte + geração com imagem de referência: é assim que o
  produto mantém a MESMA identidade em vários ângulos e que se faz re-grade)
  e `flux` (flux-dev · barato · texturas/plates).
- **Vídeo (i2v, tier cinema):** escolher o modelo vigente via `/find-models`
  (seedance / wan / kling / hailuo — disponibilidade muda; checar na hora) e
  rodar via `/run-models`, passando o hero aprovado como imagem inicial.
- **Remove background:** modelo de bg-removal no Replicate via `/find-models`
  (ou rembg local se instalado).
- **3D (GLB, tier spectacle):** modelo image-to-3D no Replicate via
  `/find-models` (família trellis/hunyuan3d). Verificar a imagem-fonte ANTES
  do passo caro.
- Upscale/outpaint: idem, `/find-models` quando precisar.

Se um modelo rejeitar parâmetro, ler o schema dele na API (`/run-models`
ensina) — cicatriz nano-banana-pro: `resolution` é `"2K"`, não `"2k"`.
...
HTML). Download everything into `assets/` do projeto and reference
relative (`assets/...`). Downscale: hero ≤2k, cutouts ~800px, icons ~256px.
...
1. **Hero visual** — the centerpiece. Generate 2 candidates, pick one, and
   ...
   **CICATRIZ 2026-07-14 — não peça o gerador pra "reservar" espaço de texto.**
   Prompt com "left third darker for headline text" faz o modelo **queimar um
   bloco chapado morto** no terço — que parece imagem cortada / "não é wide" no
   hero full-bleed. Gere a cena **preenchendo o quadro inteiro edge-to-edge**
   (16:9 pra hero largo) e resolva a legibilidade do texto **no CSS** (um véu/
   gradiente por cima), nunca dentro do pixel. E confira o arquivo gerado: bloco
   morto/borda queimada = re-roll, não conserta no CSS.
...
7. **Head kit (the full favicon/meta set — the FILES exist on every build).**
   ...
   - **Meta de descoberta** (obrigatória em toda página indexável):
     `<meta name="robots" content="max-image-preview:large, max-snippet:-1, max-video-preview:-1">`.
     É o gate do card de imagem grande na busca e da elegibilidade ao Discover.
     Cicatriz 2026-07-25 (guiloureiro.com.br): faltava nas 129 páginas → Discover
     com ZERO impressão em 90 dias, mesmo com schema, autor e imagem 1344px OK.
     Duas regras irmãs: **uma só** meta `robots` por página (página `noindex` NÃO
     recebe esta — conflita), e a imagem de `og:image` precisa ter **≥1200px de
     largura no arquivo** (medir o arquivo; o atributo `width` do HTML mente).
...
7. **Hero video — cinema tier REQUIRES the scroll-scrub, not the loop.** A
   ...
   **interactive scrub**: the user's scroll literally plays a film. Recipe
   (proven end-to-end):
   - Generate the approved hero image first, then image-to-video no Replicate
     (modelo i2v vigente via `/find-models`, hero como imagem inicial) with a
     slow, cut-free ~5s motion whose progression maps to scroll: push-in +
     rack focus soft→sharp, subject turn/eyes to camera, light sweep, object
     assembling. START state ≠ END state, or the scrub has no payoff. Prompt
     "no cuts, no camera shake, slow steady motion only" and name the
     grade/hexes. ⚠️ Vídeo custa mais que imagem — avisar o custo estimado
     ANTES de gerar.
   - ffmpeg: `-vf "fps=20,scale=1280:-2" -q:v 4` → ~100 jpg frames (a 5s clip
     lands ~4MB) → `assets/frames/hero/`.
   - Canvas image-sequence renderer bound to the pinned hero's ScrollTrigger
     ...
     pin.
   - **Input pro modelo i2v:** o Replicate aceita URL pública ou data URI
     base64 do arquivo local (o gen_images.py mostra o padrão de refs).
...
8. **3D subject** — approved hero image → modelo image-to-3D no Replicate
   (`/find-models`) → `.glb` for a three.js scene (verify the source image
   BEFORE the expensive 3D step).
...
- **Product universe** — multiple angles + lifestyle context shots derived
  ...
  (nano-banana-pro com `refs`), so every view is recognizably the SAME product.
...
- **Diagrams as images** — process flows, architecture "how it works" panels
  ...
  divs.
- **Brand sound (sparingly)** — um sting curto via modelo de áudio no
  Replicate (`/find-models`) behind an explicit user-triggered toggle (never
  autoplay); fits music/game/creative-studio briefs.
...
- **Submit early, never idle.** Todo o kit vai numa chamada em lote do
  gen_images.py (em background) logo depois dos boards escolhidos; construir
  o HTML enquanto renderiza. A build that waits on renders is mis-sequenced.
...
- **Rejects live in `refs/`, not `assets/`.** Multi-candidate generations
  (e.g. the 2 hero candidates) leave exactly one winner in `assets/`;
  move unpicked candidates and intermediates to `refs/` so the
  "everything in assets/ is referenced" gate stays clean.
